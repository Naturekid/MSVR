	...
	if (!fragMap.empty())
	{
		...
		for (itr=fragMap.begin(); itr!=fragMap.end();itr++)
		{
			...
			if (CURRENT_TIME - itr_iner->second.recv_time > MAX_BUFFER_TIME)
			{
				...
				sendNAK(bh_nak->prev, itr->first);  //暂时考虑将整个BUNDLE重传
			}
		}
	}
	//
...
//	hdr_ip* iph=hdr_ip::access(p);
	...
	//按目的地坐标和效用值的比对算出最佳下一跳, if dst node is in my nblist ,then send pkt straight
...
/*
*	处理确认，删除缓存对应数据
*	（根据Bundleid删除，节点收到完整的BUNDLE才会发ack）
*/
...
*		短路包是按照(src, dst) （上一跳DTN???, 目的地）配对发送
*/
...
//	Packet* newpkt = midBundleBuffer->lookup_by_SrcDst(bh->src, bh->dst);
	...
	while (n < midBundleBuffer->length()) {
			...
			if (bh->src == disbh->src && bh->dst == disbh->dst){
				myBundleBuffer->enque(pkt);//收到确认后再删除midBuffer
			}
			n++;
		}
	//
...
/*
*	组装并返回Bundle，【直接存入队列而不是返回】最后【清空相应表项】
*/
